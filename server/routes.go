...
func registerRoutes(r chi.Router, application *app.Application, authMiddleware func(http.Handler) http.Handler) {
	...
	r.With(authMiddleware).Mount("/jobs", job.Routes(application.JobStore, application.Publisher, application.JobCreatedTopicID, application.InterviewScheduledTopicID))
